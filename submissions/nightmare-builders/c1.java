...
:two: Otherwise print `:(`
:three: Crossing beams of different turrets will explode, so print `:/`
...
> / <  @ \ @
  @      ^
```
> first turret hits the bottom target, second hits the top one
> bottom turrets hits the left target, top turret hits the right one

These turrets are V3.0, so they are impervious to lasers and block its way; just like the Companion Cube `#`.
